...
export default function HeroSection() {
  ...
  return (
    <section className="relative flex flex-col overflow-hidden" style={{ minHeight: "100svh" }}>
      {/* ── Background ── */}
      <div className="absolute inset-0 pointer-events-none" style={{ backgroundColor: "#050814" }}>
        {/* Mobile: cover so train fills screen */}
        <div
        ...
        />
        {/* Desktop: original wide positioning */}
        <div
        ...
        {/* Mobile overlay: top light → bottom heavy so content is readable */}
        <div
          className="absolute inset-0 sm:hidden"
          style={{
            background:
              "linear-gradient(to bottom, rgba(5,8,20,0.25) 0%, rgba(5,8,20,0.55) 45%, rgba(5,8,20,0.92) 75%, #050814 100%)",
          }}
        />
        {/* Desktop overlay: left heavy for text legibility */}
        <div
          className="absolute inset-0 hidden sm:block"
          style={{
            background:
      ...
      {/* ── LEXCE mascot (desktop only) ── */}
      <motion.div
        initial={{ opacity: 0, x: 60, scale: 0.9 }}
        animate={{ opacity: 1, x: 0, scale: 1 }}
        transition={{ duration: 1, delay: 0.15, ease: "easeOut" }}
        className="hidden lg:block absolute pointer-events-none"
        style={{ top: "16vw", right: "8%", width: "38vw", height: "44vw", zIndex: 2 }}
      >
        <div
          className="absolute inset-0"
          style={{ transform: "scaleX(0.8) translateY(6%)", opacity: 0.22, filter: "blur(22px) brightness(2.2)" }}
        >
      ...
      {/* ── Hero content ── */}
      {/* Mobile: push to bottom of the image. Desktop: center vertically */}
      <div className="relative z-10 mx-auto w-full max-w-7xl px-6 sm:px-10 flex-1 flex flex-col justify-end sm:justify-center pb-8 pt-[55vw] sm:pt-16 sm:pb-8">
        <div className="max-w-[480px]">
          ...
          >
            <div
              className="text-6xl sm:text-8xl lg:text-9xl leading-none"
              style={{ fontFamily: "var(--font-display)", color: "var(--text-primary)" }}
            >
              VIDI
              <span style={{ color: "var(--accent-gold)", textShadow: "0 0 40px rgba(245,197,24,0.5)" }}>26</span>
            </div>
            <div
